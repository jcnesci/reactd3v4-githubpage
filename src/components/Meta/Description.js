...
class Description extends Component {
	...
	get countyFragment() {
		...
		let ordered = _.sortBy(
										_.keys(byCounty)
											...
											.filter(d => d.length / this.props.data.length > 0.01),			//?: what's this mean?
										items => d3mean(items,
		...
		// We use basically the same process as county (above) to get the best city.
		...
		ordered = _.sortBy(_.keys(byCity)
												...
												items => d3mean(items, d => d.base_salary) - countyMedian);		//?: what's this mean?

		best = ordered[ordered.length - 1];		//?: is best a city now, or a state?

		const city = S(best[0].city).titleCase().s + `, ${best[0].USstate}`, 		//?: what is titleCase().s?
	...
	render() {
		...
		return (
			<p className="lead">
				{this.yearsFragment ? this.yearsFragment : "Since 2012"} the {this.USstateFragment} tech industry {this.yearsFragment ? "sponsored" : "has sponsored"} <b>{format(this.props.data.length)} {this.jobTitleFragment}</b>{this.previousYearFragment}. Most of them paid <b>${format(mean - deviation)} to ${format(mean + deviation)}</b> per year. {this.countyFragment}
			</p>
		);
	}
}
