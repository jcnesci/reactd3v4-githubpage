...
export const loadAllData = (callback = _.noop) => {
	d3.queue()
		...
		.await((error, us, countyNames, medianIncomes, techSalaries, USstateNames) => {
			countyNames = countyNames.map(({ id, name }) => ({ id: Number(id), name: name }));		//ps: should've been cleaned beforehand in a parsing function like those above, but Swizec forgot to and did it here instead.
			...
			techSalaries = techSalaries.filter(d => !_.isNull(d));			//ps: there actually aren't any null datapoints.
			callback({
				usTopoJson: us,
